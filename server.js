...
function connected(socket) {
  if (Object.keys(playerPos).length === 0) {
    console.log("First client join the room");
    serverBalls[socket.id] = new Capsule(80, 270, 150, 270, 25, 10);
    serverBalls[socket.id].maxSpeed = 3;
    ...
    console.log("Second Client join the room");
    ...
    io.emit("updateFootabll", {
      x: football.pos.x,
      y: football.pos.y
    })
  } else if (Object.keys(playerPos) > 1) {
    console.log("Too many players");
    return false;
  }
  io.emit('updateConnection', playerPos);
...
function serverLoop() {
  ...
  if (football === undefined) {
    console.log("Waiting for 2 client to join");
  } else {
    footballPos = {
      x: football.pos.x,
      y: football.pos.y
  ...
  io.emit("updateFootabll", footballPos);
}
  ...
  if (football.pos.x < 45) {
    for (let id in serverBalls) {
      if (serverBalls[id].no === 2) {
        ...
        scoreId = id;
        console.log('score for second player');
      }
    }
  }
  if (football.pos.x > 595) {
    for (let id in serverBalls) {
      if (serverBalls[id].no === 1) {
        serverBalls[id].score++;
        scoreId = id;
        console.log('score for first player');
      }
    }
  }
  gameSetUp();
  io.emit('updateScore', scoreId);
}
